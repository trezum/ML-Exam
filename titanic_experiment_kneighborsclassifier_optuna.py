...
def optuna_search():
    ...
    if path.exists("kneighborsclassifier_study.pkl"):
        study = joblib.load('kneighborsclassifier_study.pkl')
    else:
        study = optuna.create_study(direction='maximize')

    study.optimize(objective, n_trials=2000)

    joblib.dump(study, 'kneighborsclassifier_study.pkl')
...
# Best params:
# {'n_neighbors': 9, 'weights': 'distance', 'algorithm': 'ball_tree', 'metric': 'manhattan', 'missing_data_way': 'median', 'scaler': 'PowerTransformer'}
# Best value:
# 0.8235294117647058
